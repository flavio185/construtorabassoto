...
def nota_list(request):
    ...
    #Seta vareiaveis caso náo exista notas ainda.
    projeto=None
    notas=None
...
def nota_filter(request):
    ...
    if 'servico_terceirizado' in request.GET and request.GET['servico_terceirizado']:
        servico_terceirizado = request.GET['servico_terceirizado']
        notas = notas.filter(servico_terceirizado=servico_terceirizado)
        query += ".filter(servico_terceirizado='"+servico_terceirizado+"')"
    else:
        servico_terceirizado=None
...
def nota_nova(request):
    if request.method == "POST":
        ...
        print(form)
        #print(form.cleaned_data['servico_terceirizado'])
        if form.is_valid():
            #numero = form.cleaned_data['numero']
            #data = form.cleaned_data['data']
            #descricao = form.cleaned_data['descricao']
            print(form.cleaned_data['valor'])
            ##if servico_terceirizado:
             #   print("veio por servico.")
            #else:
            #    print("veio de notas.")
            form.save()
            #form = {
        ...
        servico = request.get_full_path().split('=')[1] if "servico_terceirizado" in request.get_full_path() else 'None'
        print(servico)
        form = NotaForm()
        form.fields['servico_terceirizado'].initial = servico
        print(form)
        #form = {
        #    'form': form,
        #    'servico': servico,
...
"""
views para servico terceirizado / notas
"""
def get_nota_servico_function(filter):
    notas = Nota.objects.filter(servico_terceirizado=filter)
    return notas

def get_servico_nota_function(filter):
    servico_terceirizado = ServicoTerceirizado.objects.filter(pk=filter)
    return servico_terceirizado

def servico_nota(request, pk):
    servicos_terceirizados = get_servico_nota_function(pk)
    notas = get_nota_servico_function(pk)
    context = {
        'servicos_terceirizados': servicos_terceirizados,
        'notas': notas,
        'pk': pk
    }
    return render(request, 'servicos/servico_nota_list.html', context)
