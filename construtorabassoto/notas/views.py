...
def nota_list(request):
    ...
    if projetos:
        ...
        total_depositos = get_soma_depositos()
        total_notas_caixa = get_soma_notas()
        total_caixa = total_depositos - total_notas_caixa
        ...
        context = { 'allProjetos': allProjetos,
                    ...
                    'total_notas_caixa':total_notas_caixa,
...
def get_soma_depositos():
    ...
    for projeto in allProjetos:
        try:
            ...
            #total_depositos = deposito.aggregate('valor')
            #print(total)
        except:
            #nao existe deposito no sistema
            deposito=""
...
def get_soma_notas():
    ...
    for projeto in allProjetos:
        ...
        if deposito != "":
            ...
            notas = notas.filter(deposito=deposito)
            if notas:
                total_notas_deposito = notas.aggregate(Sum('valor'))
                total_notas_deposito = total_notas_deposito['valor__sum']
                total_notas_caixa += total_notas_deposito
    
    return(total_notas_caixa)
"""
...
def nota_nova(request):
    if request.method == "POST":
        ...
        servico = request.get_full_path().split('=')[1] if "servico_terceirizado" in request.get_full_path() else ''
        form = NotaForm()
...
def servico_detail(request, pk):
    ...
    servico_terceirizado = ServicoTerceirizado.objects.filter(pk=pk)
